...
export function usePagination(defaultPageSize = 20) {
  ...
  const hasNext = computed(() =>
    总数.value > 0 ? 页.value * pageSize.value < 总数.value : dataLength.value >= pageSize.value
  );
  const 总页数 = computed(() =>
    总数.value > 0 ? Math.ceil(总数.value / pageSize.value) : 0
  );
  ...
  return { 页, pageSize, 总数, 总页数, hasNext, prev, next, reset, 跳转到, setDataLength, set总数 };
}
