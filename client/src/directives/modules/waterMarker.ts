/*
  需求：给整个页面添加背景水印。

  思路：
    1、使用 canvas 特性生成 base64 格式的图片文件，设置其字体大小，颜色等。
    2、将其设置为背景图片，从而实现页面或组件水印效果

  使用：设置水印文案，颜色，字体大小即可
  <div v-waterMarker="{text:'版权所有',textColor:'rgba(180, 180, 180, 0.4)'}"></div>
*/

import type { Directive, DirectiveBinding } from "vue";

type WaterMarkerOptions = { text: string; textColor?: string; font?: string };

/** 生成水印背景并挂载到元素 */
function addWaterMarker(el: HTMLElement, binding: DirectiveBinding<WaterMarkerOptions>): void {
    // 水印文字，父元素，字体，文字颜色
    const {
        text,
        textColor = "rgba(180, 180, 180, 0.3)",
        font = "16px Microsoft JhengHei",
    } = binding.value ?? {};
    const can: HTMLCanvasElement = document.createElement("canvas");
    el.appendChild(can);
    can.width = 205;
    can.height = 140;
    can.style.display = "none";
    const cans = can.getContext("2d") as CanvasRenderingContext2D;
    cans.rotate((-20 * Math.PI) / 180);
    cans.font = font;
    cans.fillStyle = textColor;
    cans.textAlign = "left";
    cans.textBaseline = "Middle" as CanvasTextBaseline;
    cans.fillText(text, can.width / 10, can.height / 2);
    el.style.backgroundImage = `url(${can.toDataURL("image/png")})`;
}

const waterMarker: Directive = {
    mounted(el: HTMLElement, binding: DirectiveBinding) {
        addWaterMarker(el, binding);
    },
};

export default waterMarker;
