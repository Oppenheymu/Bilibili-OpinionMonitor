...
const 分组列表: 配置分组[] = [
  {
    ...
    字段: [
      ...
      },
      {
        ...
        label: "分析预算上限",
        type: "数字",
        提示: "每轮分析的 LLM 调用次数上限（0=不限）。现象级舆情评论暴增时防止费用爆炸，超出部分留待下轮按热度优先分析",
        默认值: 500,
        校验规则(v) {
          const n = Number(v);
          if (n < 0) return "不能为负数";
      ...
      },
      {
        ...
        label: "B站请求间隔",
        type: "数字",
        提示: "采集请求之间的最小间隔（毫秒），自动附加随机抖动。间隔太短易触发 B站风控导致数据被截断，建议 ≥800",
        默认值: 1200,
        校验规则(v) {
          const n = Number(v);
      ...
      },
      {
        ...
        label: "失败重试次数",
        type: "数字",
        提示: "接口失败时的指数退避重试次数（1s→2s→4s…上限 30s）。风控信号会自动触发 5 分钟降速模式",
        默认值: 3,
        校验规则(v) {
          const n = Number(v);
