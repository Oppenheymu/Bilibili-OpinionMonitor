...
/**
 * 采集互斥：防止调度器与手动触发并发跑同一类采集。
 * 采集请求经 rateLimit 全局串行，重复并发触发会互相排队，造成"卡住"假象。
 */
const running: { videos: boolean; comments: boolean; dynamics: boolean } = {
    videos: false,
    comments: false,
    dynamics: false,
};
...
export async function collectVideos(): Promise<{ videos: number }> {
    if (running.videos) {
        console.log("[采集] 视频采集正在进行中，跳过本次重复触发");
        return { videos: 0 };
    }
    running.videos = true;
    ...
    try {
        ...
        console.log(`[采集] 视频完成：接口返回 ${videoCount} 条`);
        return { videos: videoCount };
    } finally {
        running.videos = false;
    }
}
...
export async function collectComments(): Promise<{ comments: number }> {
    if (running.comments) {
        console.log("[采集] 评论采集正在进行中，跳过本次重复触发");
        return { comments: 0 };
    }
    ...
    try {
        ...
        console.log(`[采集] 评论完成：新增 ${commentCount} 条`);
...
export async function collectDynamics(): Promise<{ dynamics: number }> {
    if (running.dynamics) {
        console.log("[采集] 动态采集正在进行中，跳过本次重复触发");
        return { dynamics: 0 };
    }
    running.dynamics = true;
    ...
    try {
        ...
        for (const task of tasks) {
            try {
                const list = await collector.fetchUpDynamics(
                    Number(task.target),
                    params.dynamicPages,
                );
        ...
        console.log(`[采集] 动态完成：新增 ${dynamicCount} 条`);
        return { dynamics: dynamicCount };
    } finally {
        running.dynamics = false;
    }
}
